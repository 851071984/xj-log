package iie.cas.po;

public class MAILGKZBLogPo {
	private String bhsj;

	private String pzid;

	private Integer flid;

	private Integer xzid;

	private Integer lhid;

	private String ydy;

	private String mddy;

	private Short pzlx;

	private Short gklb;

	private Byte yyzl;

	private Integer qdjip;

	private String sip;

	private String dip;

	private Integer sport;

	private Integer dport;

	private Integer protocol;

	private String protoId;

	private String detailId;

	private String mailSender;

	private Integer receverNum;

	private Integer ccNum;

	private String mailSubject;

	private Integer accessory;

	private String fileName;

	private String fileContent;

	private String pwd;

	private String fjfws;

	private Integer fjfwsSzd;

	private String sjfws;

	private String sjfwsSzd;

	private String xylx;

	private String gjz;

	private Integer yys;

	private Integer yl1;

	private Integer yl2;

	private String yl3;

	private String yl4;

	private String yl5;

	private String mailRecever;

	private String mailCc;

	private String content;
	private String label;

	public String getLabel() {
		return label;
	}

	public void setLabel(String label) {
		this.label = label;
	}

	public String getMailRecever() {
		return mailRecever;
	}

	public void setMailRecever(String mailRecever) {
		this.mailRecever = mailRecever;
	}

	public String getMailCc() {
		return mailCc;
	}

	public void setMailCc(String mailCc) {
		this.mailCc = mailCc;
	}

	public String getContent() {
		return content;
	}

	public void setContent(String content) {
		this.content = content;
	}

	public String getBhsj() {
		return bhsj;
	}

	public void setBhsj(String bhsj) {
		this.bhsj = bhsj;
	}

	public String getPzid() {
		return pzid;
	}

	public void setPzid(String pzid) {
		this.pzid = pzid;
	}

	public Integer getFlid() {
		return flid;
	}

	public void setFlid(Integer flid) {
		this.flid = flid;
	}

	public Integer getXzid() {
		return xzid;
	}

	public void setXzid(Integer xzid) {
		this.xzid = xzid;
	}

	public Integer getLhid() {
		return lhid;
	}

	public void setLhid(Integer lhid) {
		this.lhid = lhid;
	}

	public String getYdy() {
		return ydy;
	}

	public void setYdy(String ydy) {
		this.ydy = ydy;
	}

	public String getMddy() {
		return mddy;
	}

	public void setMddy(String mddy) {
		this.mddy = mddy;
	}

	public Short getPzlx() {
		return pzlx;
	}

	public void setPzlx(Short pzlx) {
		this.pzlx = pzlx;
	}

	public Short getGklb() {
		return gklb;
	}

	public void setGklb(Short gklb) {
		this.gklb = gklb;
	}

	public Byte getYyzl() {
		return yyzl;
	}

	public void setYyzl(Byte yyzl) {
		this.yyzl = yyzl;
	}

	public Integer getQdjip() {
		return qdjip;
	}

	public void setQdjip(Integer qdjip) {
		this.qdjip = qdjip;
	}

	public String getSip() {
		return sip;
	}

	public void setSip(String sip) {
		this.sip = sip;
	}

	public String getDip() {
		return dip;
	}

	public void setDip(String dip) {
		this.dip = dip;
	}

	public Integer getSport() {
		return sport;
	}

	public void setSport(Integer sport) {
		this.sport = sport;
	}

	public Integer getDport() {
		return dport;
	}

	public void setDport(Integer dport) {
		this.dport = dport;
	}

	public Integer getProtocol() {
		return protocol;
	}

	public void setProtocol(Integer protocol) {
		this.protocol = protocol;
	}

	public String getProtoId() {
		return protoId;
	}

	public void setProtoId(String protoId) {
		this.protoId = protoId;
	}

	public String getDetailId() {
		return detailId;
	}

	public void setDetailId(String detailId) {
		this.detailId = detailId;
	}

	public String getMailSender() {
		return mailSender;
	}

	public void setMailSender(String mailSender) {
		this.mailSender = mailSender;
	}

	public Integer getReceverNum() {
		return receverNum;
	}

	public void setReceverNum(Integer receverNum) {
		this.receverNum = receverNum;
	}

	public Integer getCcNum() {
		return ccNum;
	}

	public void setCcNum(Integer ccNum) {
		this.ccNum = ccNum;
	}

	public String getMailSubject() {
		return mailSubject;
	}

	public void setMailSubject(String mailSubject) {
		this.mailSubject = mailSubject;
	}

	public Integer getAccessory() {
		return accessory;
	}

	public void setAccessory(Integer accessory) {
		this.accessory = accessory;
	}

	public String getFileName() {
		return fileName;
	}

	public void setFileName(String fileName) {
		this.fileName = fileName;
	}

	public String getFileContent() {
		return fileContent;
	}

	public void setFileContent(String fileContent) {
		this.fileContent = fileContent;
	}

	public String getPwd() {
		return pwd;
	}

	public void setPwd(String pwd) {
		this.pwd = pwd;
	}

	public String getFjfws() {
		return fjfws;
	}

	public void setFjfws(String fjfws) {
		this.fjfws = fjfws;
	}

	public Integer getFjfwsSzd() {
		return fjfwsSzd;
	}

	public void setFjfwsSzd(Integer fjfwsSzd) {
		this.fjfwsSzd = fjfwsSzd;
	}

	public String getSjfws() {
		return sjfws;
	}

	public void setSjfws(String sjfws) {
		this.sjfws = sjfws;
	}

	public String getSjfwsSzd() {
		return sjfwsSzd;
	}

	public void setSjfwsSzd(String sjfwsSzd) {
		this.sjfwsSzd = sjfwsSzd;
	}

	public String getXylx() {
		return xylx;
	}

	public void setXylx(String xylx) {
		this.xylx = xylx;
	}

	public String getGjz() {
		return gjz;
	}

	public void setGjz(String gjz) {
		this.gjz = gjz;
	}

	public Integer getYys() {
		return yys;
	}

	public void setYys(Integer yys) {
		this.yys = yys;
	}

	public Integer getYl1() {
		return yl1;
	}

	public void setYl1(Integer yl1) {
		this.yl1 = yl1;
	}

	public Integer getYl2() {
		return yl2;
	}

	public void setYl2(Integer yl2) {
		this.yl2 = yl2;
	}

	public String getYl3() {
		return yl3;
	}

	public void setYl3(String yl3) {
		this.yl3 = yl3;
	}

	public String getYl4() {
		return yl4;
	}

	public void setYl4(String yl4) {
		this.yl4 = yl4;
	}

	public String getYl5() {
		return yl5;
	}

	public void setYl5(String yl5) {
		this.yl5 = yl5;
	}
}